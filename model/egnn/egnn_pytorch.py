...
class EGNN(nn.Module):
    ...
    def forward(self, feats, coors, edges=None, mask=None, adj_mat=None):
        ...
        if use_nearest:
            ...
            if num_nearest > ranking.shape[1]: num_nearest = ranking.shape[1]
...
class EGNN_Network(nn.Module):
    def __init__(self, *, depth, dim, num_tokens=None, num_edge_tokens=None, num_positions=None, edge_dim=0,
                 ...
                 global_linear_attn_dim_head=64, num_global_tokens=4, aggregate=True, num_nearest_neighbors, **kwargs):
        super().__init__()
        ...
        if exists(num_tokens):
            ...
            # Hamiltonian的特征维度为4
            self.token_emb = nn.Sequential(nn.Linear(4, dim), nn.ReLU(), nn.Linear(dim, dim))
