...
class SimulationDataset(object):
    ...
    def simulate(self, ns, key=0):
        ...
        times = self.times
        G = self.G
        if self.extra_potential is not None: vex = vmap(self.extra_potential, 0, 0)
        ...
        def total_potential(xt):
            ...
            for i in range(n - 1):
                if sim in ['string', 'string_ball']:
                    ...
                    # 计算所有两两节点之间的势能
                    sum_potential = sum_potential + G * vp(xt[i], xt[i + 1:]).sum()
            if self.extra_potential is not None: sum_potential = sum_potential + vex(xt).sum()
        ...
        def make_sim(key):
            if sim in ['string', 'string_ball']:
                ...
                x0 = x0.at[s_[..., 0]].set(np.arange(n) + x0[..., 0] * 0.5)
                x0 = x0.at[s_[..., 2:3]].set(0.0)
            else:
                x0 = random.normal(key, (n, total_dim))
                x0 = x0.at[s_[..., -1]].set(np.exp(x0[..., -1]))  # all masses set to positive
                if sim in ['charge', 'superposition']:
                    x0 = x0.at[s_[..., -2]].set(np.sign(x0[..., -2]))  # charge is 1 or -1
